...
**关键点**：
- 首次执行延迟**1秒**，快速启动新浏览器或检测老浏览器重连
- 每次检查前都判断 `_browsers.ContainsKey(configId)`
- 如果已连接，就不启动

#### 5. 时间线示例（主程序重启，老浏览器在运行）

| 时间 | 事件 | 说明 |
|------|------|------|
| 0s | 主程序启动 | Socket 服务器启动，监控任务启动（1秒后首次执行） |
| 0.1s | 飞单开关=true | 调用 `StartBrowser(1)`，标记配置1需要启动 |
| 0.2s | `StartBrowser` 等待 | 检查 `_browsers` 为空，开始等待连接（最多3秒） |
| 1s | 老浏览器重连 | 检测到断线，1秒后重连成功 |
| 1.1s | `OnBrowserConnected` | 自动创建 `BrowserClient`，添加到 `_browsers[1]` |
| 1.2s | `StartBrowser` 返回 true | 检测到 `_browsers` 中有配置1，返回成功 |
| 1.3s | 监控任务首次执行 | 检查配置1，发现已在 `_browsers` 中，**不启动新浏览器** |

**结果**：✅ 只有1个浏览器，老浏览器成功重连，等待时间约1秒
...
| 0.1s | 飞单开关=true | 调用 `StartBrowser(1)`，标记配置1需要启动 |
| 0.2s | `StartBrowser` 等待 | 检查 `_browsers` 为空，开始等待连接（最多3秒） |
| 1s | 监控任务首次执行 | 检查配置1，**不在 `_browsers` 中**，启动新浏览器 |
| 1.1s | 新浏览器启动 | 创建进程，2秒后连接 Socket |
| 3.1s | `OnBrowserConnected` | 新浏览器连接成功，添加到 `_browsers[1]` |
| 3.2s | `StartBrowser` 返回 true | 检测到 `_browsers` 中有配置1，返回成功 |
| 4s | 监控任务第2次执行 | 检查配置1，已在 `_browsers` 中，**不重复启动** |

**结果**：✅ 只有1个浏览器，新浏览器成功启动，等待时间约3秒
...
_monitorTimer = new System.Threading.Timer(
    MonitorBrowsers, 
    null, 
    TimeSpan.FromSeconds(1),  // 首次延迟1秒
    TimeSpan.FromSeconds(3)   // 每3秒执行一次
);

// StartBrowser：标记并等待连接
public async Task<bool> StartBrowser(int configId)
{
    lock (_lock)
    {
