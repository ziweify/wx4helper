    ...
    public class BinggoOrderValidator
    {
            ...
            out string errorMessage)
        {
            ...
            try
            {
                ...
                {
                    errorMessage = $"您的账户状态:{member.State}，无法下注";
                    return false;
                }
                ...
                foreach (var kvp in currentOrderGrouped)
                {
                    ...
                    _logService.Info("BinggoOrderValidator", $"   - 检查投注项: {key}, 本单金额={currentAmount}");
                    ...
                    if (currentAmount < (decimal)minBet)
                    {
                        ...
                        _logService.Warning("BinggoOrderValidator", $"❌ {errorMessage}（实际: {currentAmount}）");
                        return false;
                    }
