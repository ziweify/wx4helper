...
namespace F5BotV2.BetSite.HongHai
{
        ...
        , INotifyPropertyChanged
    {
        ...
        //测试账号: wwww11
        //    密码: Aaa123
        public TongBaoPcMember(IBetBrowserBase browser)
        {
        ...
        public int LoginAsync(string name, string pass, IBetBrowserBase browser)
        {
            ...
            if (cts == null)
            {
                ...
                Task.Factory.StartNew(async (object hx666) => {
                    ...
                    {
                        TongBaoPcMember hx = hx666 as TongBaoPcMember;
                        bool response = true;
                        try
                        {
        ...
        {
            //校验url.必须要带 / 结尾
            //PC的要去掉末尾的, 只保留纯的域名, 截断到到第一个/结尾的地方
            string root_addr = Regex.Match(url, @"^(https?://[^/]+)").Value;
            return betApi.SetRootUrl(root_addr);
        }
