...
### 核心需求
1. **自定义脚本语言**：不是简单的JavaScript，而是支持自定义流程和功能
2. **运行时编辑**：用户可以在运行时修改和调试脚本
3. **编辑器功能**：
   - ✅ 断点调试
   - ✅ 语法高亮
   - ✅ 语法错误检查
...
| 方案 | 优点 | 缺点 | 推荐度 |
|------|------|------|--------|
| **Lua (NLua/MoonSharp)** | ✅ 轻量级、快速<br>✅ 易于嵌入<br>✅ 调试支持好<br>✅ 语法简单 | ⚠️ 需要学习Lua语法 | ⭐⭐⭐⭐⭐ |
| **Python (IronPython)** | ✅ 语法友好<br>✅ 功能强大<br>✅ 生态丰富 | ❌ 性能一般<br>❌ 调试支持有限<br>❌ 依赖较大 | ⭐⭐⭐ |
| **JavaScript (V8/ChakraCore)** | ✅ 用户熟悉<br>✅ 性能好 | ❌ 调试支持复杂<br>❌ 需要额外引擎 | ⭐⭐⭐ |
| **C# Script (Roslyn)** | ✅ 直接使用C#语法<br>✅ 类型安全<br>✅ 调试支持好 | ❌ 需要编译<br>❌ 性能开销 | ⭐⭐⭐⭐ |
| **自定义DSL** | ✅ 完全定制<br>✅ 符合业务需求 | ❌ 开发成本高<br>❌ 需要实现解析器 | ⭐⭐ |

### 推荐方案：**Lua (MoonSharp)**

**理由：**
1. ✅ **轻量级**：体积小，性能好
2. ✅ **易于嵌入**：MoonSharp 是纯 C# 实现，无需外部依赖
3. ✅ **调试支持**：MoonSharp 支持断点、单步调试
4. ✅ **语法简单**：学习成本低
5. ✅ **功能绑定**：可以轻松绑定 C# 函数和对象
6. ✅ **运行时修改**：支持动态执行和修改

**备选方案：C# Script (Roslyn)**
- 如果用户更熟悉 C#，可以使用 Roslyn 脚本
- 支持完整的 C# 语法和调试
...
#### ScriptEngine（脚本引擎接口）
```csharp
namespace 永利系统.Services.Scripting
{
    /// <summary>
    /// 脚本引擎接口
    /// </summary>
...
**理由：**
1. ✅ **功能完整**：语法高亮、代码折叠、自动完成
2. ✅ **断点支持**：可以标记断点行
3. ✅ **错误标记**：可以标记错误行
4. ✅ **WinForms集成**：原生支持 WinForms
5. ✅ **开源免费**：MIT 许可证
6. ✅ **性能好**：适合大文件编辑

**备选方案：**
- **AvalonEdit**（需要 WPF 集成）
- **Monaco Editor**（需要 WebView2）
- **DevExpress Code Editor**（如果项目已有 DevExpress）
...
1. **开箱即用**：创建即用，无需额外配置
2. **高度封装**：所有功能内置，外部只需简单调用
3. **可复用**：可在多个地方使用，统一体验
4. **设计器支持**：可在设计器中直接使用和预览

### 控件位置

**建议位置**：`永利系统/Views/Shared/Controls/ScriptEditorControl.cs`

**理由**：
- 放在 Shared 位置，便于多个模块复用
- 独立的控件，不依赖特定业务逻辑
- 可以在设计器中直接拖放使用

---

### ScriptEditorControl（脚本编辑器控件 - 完整封装版）

```csharp
namespace 永利系统.Views.Shared.Controls
{
    /// <summary>
    /// 脚本编辑器控件（完整封装，开箱即用）
    /// 
    ...
    public partial class ScriptEditorControl : XtraUserControl
    {
        ...
        {
            // 配置 ScintillaNET
            scintilla.Lexer = Lexer.Lua; // 使用 Lua 语法高亮
            ...
            // 订阅事件
            scintilla.MarginClick += Scintilla_MarginClick; // 断点点击
            scintilla.TextChanged += Scintilla_TextChanged; // 文本变更（实时验证）
        }
        ...
        {
            scintilla.MarkerAdd(line, 1); // 使用标记1（错误标记）
...
```csharp
// 在设计器中拖放 ScriptEditorControl 到窗体
// 或者代码中创建
...
### ScriptEditorControl.Designer.cs（设计器中创建）
...
### 2. 简单API
- ✅ 公共属性：`ScriptText`, `EnableBreakpoints`, `ShowLineNumbers` 等
- ✅ 公共方法：`ExecuteScript()`, `ValidateScript()`, `BindFunction()` 等
- ✅ 公共事件：`OnValidationError`, `OnError`, `OnBreakpointHit` 等
...
1. **首次使用**：控件创建后自动初始化，无需额外配置
2. **脚本引擎**：默认使用 Lua (MoonSharp)，可通过 `SetScriptEngine()` 更换
3. **功能绑定**：通过 `BindFunction()` 和 `BindObject()` 绑定自定义功能
4. **断点调试**：点击左边距即可设置/清除断点
5. **实时验证**：脚本修改后自动验证（延迟500ms）
    ...
    public static class ScriptFunctionInitializer
    {
        public static void InitializeFunctions(ScriptFunctionRegistry registry, BrowserWindowProxy browserProxy)
        {
            ...
            registry.RegisterFunction(
                "navigate",
                new Action<string>(url => browserProxy.ExecuteCommandAsync("重新导航", url)),
                "导航到指定URL",
            ...
            registry.RegisterFunction(
                "click",
                new Action<string>(selector => browserProxy.ExecuteCommandAsync("点击元素", selector)),
                "点击页面元素",
                "click('#submit-button')"
            );
            ...
            registry.RegisterFunction(
                ...
                {
                    var result = browserProxy.ExecuteCommandAsync("获取元素文本", selector).Result;
            ...
            registry.RegisterFunction(
                ...
                {
                    browserProxy.ExecuteCommandAsync("设置元素值", new { selector, value });
                }),
                "设置元素值",
            ...
            registry.RegisterFunction(
                "log",
                new Action<string>(msg => LoggingService.Instance.Info("脚本", msg)),
                "输出日志",
                "log('Hello World')"
            );
...
│  │   3      wait(2000)                             │   │
│  │   4      setValue('#username', username)        │   │
│  │   5      setValue('#password', password)        │   │
│  │   6      click('#login-button')                 │   │
│  │   7      wait(3000)                             │   │
│  │   8      local result = getText('.status')      │   │
│  │   9      log('登录结果: ' .. result)            │   │
│  │  10      return result                          │   │
│  │  11  end                                         │   │
│  └─────────────────────────────────────────────────┘   │
...
### UI集成
- [ ] 创建 `ScriptEditorControl`
- [ ] 创建 `ScriptEditorDialog`
- [ ] 集成到任务配置对话框
...
1. **性能优化**：实时验证使用延迟机制，避免频繁验证
2. **错误处理**：所有脚本执行都要有错误捕获
3. **线程安全**：脚本执行在后台线程，UI更新在主线程
4. **资源管理**：正确释放脚本引擎和编辑器资源
