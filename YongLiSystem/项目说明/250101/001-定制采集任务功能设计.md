# 定制采集任务功能设计文档

**📅 创建日期**: 2025-01-01  
**🎯 功能**: 定制采集任务（半自动脚本任务）  
**📍 位置**: 数据采集页面 → 右侧 groupCustTask

---

## 📋 功能概述

### 核心功能
1. **添加任务**: 点击 `buttonAddScriptTask` 弹出配置对话框，创建新的脚本任务
2. **任务管理**: 流式布局显示所有任务卡片
3. **浏览器集成**: 每个任务包含一个独立的浏览器窗口（BrowserWindow）
4. **实时缩略图**: 1秒生成一次浏览器窗口缩略图，实时更新到卡片上
5. **数据持久化**: 使用 SQLite 数据库存储任务配置，自动保存和加载

---

## 🏗️ 架构设计

### 1. 数据模型层

#### ScriptTask 类（脚本任务对象）
```csharp
namespace YongLiSystem.Models.Dashboard
{
    /// <summary>
    /// 脚本任务对象
    /// </summary>
    public class ScriptTask
    {
        /// <summary>
        /// 任务ID（数据库主键）
        /// </summary>
        public int Id { get; set; }
        
        /// <summary>
        /// 任务名称
        /// </summary>
        public string Name { get; set; } = string.Empty;
        
        /// <summary>
        /// 网址
        /// </summary>
        public string Url { get; set; } = string.Empty;
        
        /// <summary>
        /// 用户名
        /// </summary>
        public string Username { get; set; } = string.Empty;
        
        /// <summary>
        /// 密码
        /// </summary>
        public string Password { get; set; } = string.Empty;
        
        /// <summary>
        /// 是否自动登录
        /// </summary>
        public bool AutoLogin { get; set; }
        
        /// <summary>
        /// 脚本内容
        /// </summary>
        public string Script { get; set; } = string.Empty;
        
        /// <summary>
        /// 创建时间
        /// </summary>
        public DateTime CreateTime { get; set; } = DateTime.Now;
        
        /// <summary>
        /// 最后更新时间
        /// </summary>
        public DateTime UpdateTime { get; set; } = DateTime.Now;
        
        /// <summary>
        /// 是否启用
        /// </summary>
        public bool IsEnabled { get; set; } = true;
        
        // ========== 运行时属性（不存储到数据库）==========
        
        /// <summary>
        /// 浏览器窗口代理（运行时创建）
        /// </summary>
        [Ignore] // SQLite 忽略此属性
        public BrowserWindowProxy? BrowserProxy { get; set; }
        
        /// <summary>
        /// 当前状态
        /// </summary>
        [Ignore]
        public ScriptTaskStatus Status { get; set; } = ScriptTaskStatus.Stopped;
        
        /// <summary>
        /// 最新期号数据
        /// </summary>
        [Ignore]
        public string? LatestIssueData { get; set; }
        
        /// <summary>
        /// 浏览器缩略图（1秒更新一次）
        /// </summary>
        [Ignore]
        public Image? Thumbnail { get; set; }
    }
    
    /// <summary>
    /// 脚本任务状态
    /// </summary>
    public enum ScriptTaskStatus
    {
        Stopped,    // 已停止
        Running,    // 运行中
        Error       // 错误
    }
}
```

---

### 2. 数据库层

#### 数据库文件
- **路径**: 使用共同的数据库文件 `app.db`（位于 `C:\Users\[用户名]\AppData\Local\永利系统\Data\app.db`）
- **表名**: `ScriptTasks`（新增表，与现有表共存）

#### 数据库操作（集成到 DataCollectionService）
```csharp
namespace YongLiSystem.Services.Dashboard
{
    /// <summary>
    /// 数据采集服务（扩展，添加脚本任务管理功能）
    /// </summary>
    public class DataCollectionService
    {
        private readonly LoggingService _loggingService;
        private readonly string _dbPath; // 共同的数据库路径
        
        public DataCollectionService()
        {
            _loggingService = LoggingService.Instance;
            // 获取共同的数据库路径（与软件其他模块使用同一个数据库）
            var appDataPath = Path.Combine(
                Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData),
                "永利系统",
                "Data"
            );
            if (!Directory.Exists(appDataPath))
                Directory.CreateDirectory(appDataPath);
            _dbPath = Path.Combine(appDataPath, "app.db");
            
            // 初始化脚本任务表
            InitializeScriptTasksTable();
        }
        
        /// <summary>
        /// 初始化脚本任务表（在共同的数据库中创建新表）
        /// </summary>
        private void InitializeScriptTasksTable()
        {
            try
            {
                using var db = new SQLiteConnection(_dbPath);
                db.CreateTable<ScriptTask>();
                _loggingService.Info("数据采集", "脚本任务表初始化成功");
            }
            catch (Exception ex)
            {
                _loggingService.Error("数据采集", $"脚本任务表初始化失败: {ex.Message}", ex);
            }
        }
        
        /// <summary>
        /// 加载所有脚本任务
        /// </summary>
        public List<ScriptTask> LoadAllScriptTasks()
        {
            try
            {
                using var db = new SQLiteConnection(_dbPath);
                return db.Table<ScriptTask>().ToList();
            }
            catch (Exception ex)
            {
                _loggingService.Error("数据采集", $"加载脚本任务失败: {ex.Message}", ex);
                return new List<ScriptTask>();
            }
        }
        
        /// <summary>
        /// 保存脚本任务（新增或更新）
        /// </summary>
        public void SaveScriptTask(ScriptTask task)
        {
            try
            {
                task.UpdateTime = DateTime.Now;
                using var db = new SQLiteConnection(_dbPath);
                if (task.Id == 0)
                {
                    task.CreateTime = DateTime.Now;
                    db.Insert(task);
                    _loggingService.Info("数据采集", $"新增脚本任务: {task.Name}");
                }
                else
                {
                    db.Update(task);
                    _loggingService.Info("数据采集", $"更新脚本任务: {task.Name}");
                }
            }
            catch (Exception ex)
            {
                _loggingService.Error("数据采集", $"保存脚本任务失败: {ex.Message}", ex);
                throw;
            }
        }
        
        /// <summary>
        /// 删除脚本任务
        /// </summary>
        public void DeleteScriptTask(int taskId)
        {
            try
            {
                using var db = new SQLiteConnection(_dbPath);
                db.Delete<ScriptTask>(taskId);
                _loggingService.Info("数据采集", $"删除脚本任务: ID={taskId}");
            }
            catch (Exception ex)
            {
                _loggingService.Error("数据采集", $"删除脚本任务失败: {ex.Message}", ex);
                throw;
            }
        }
        
        // ... 原有的其他方法保持不变 ...
    }
}
```

---

### 3. UI 控件层

#### ScriptTaskCard（任务卡片控件）
```csharp
namespace YongLiSystem.Views.Dashboard.Controls
{
    /// <summary>
    /// 脚本任务卡片控件（流式布局中的单个卡片）
    /// 在设计器中创建UI
    /// </summary>
    public partial class ScriptTaskCard : XtraUserControl
    {
        private ScriptTask _task;
        private System.Windows.Forms.Timer? _thumbnailTimer;
        
        public ScriptTaskCard(ScriptTask task)
        {
            InitializeComponent(); // 设计器中创建UI
            _task = task;
            InitializeCard();
            StartThumbnailUpdate();
        }
        
        /// <summary>
        /// 初始化卡片显示
        /// </summary>
        private void InitializeCard()
        {
            // 显示任务信息
            lblTaskName.Text = _task.Name;
            lblUrl.Text = _task.Url;
            lblStatus.Text = _task.Status.ToString();
            
            // 绑定事件
            btnStart.Click += BtnStart_Click;
            btnStop.Click += BtnStop_Click;
            btnEdit.Click += BtnEdit_Click;
            btnDelete.Click += BtnDelete_Click;
            btnShowBrowser.Click += BtnShowBrowser_Click;
        }
        
        /// <summary>
        /// 启动缩略图更新（1秒一次）
        /// </summary>
        private void StartThumbnailUpdate()
        {
            _thumbnailTimer = new System.Windows.Forms.Timer
            {
                Interval = 1000 // 1秒
            };
            _thumbnailTimer.Tick += async (s, e) => await UpdateThumbnailAsync();
            _thumbnailTimer.Start();
        }
        
        /// <summary>
        /// 更新浏览器缩略图
        /// </summary>
        private async Task UpdateThumbnailAsync()
        {
            if (_task.BrowserProxy?.WindowHandle != IntPtr.Zero)
            {
                try
                {
                    // 使用 Windows API 捕获窗口截图
                    var thumbnail = CaptureWindowThumbnail(_task.BrowserProxy.WindowHandle);
                    if (thumbnail != null)
                    {
                        picThumbnail.Image = thumbnail;
                        _task.Thumbnail = thumbnail;
                    }
                }
                catch (Exception ex)
                {
                    // 静默处理错误，不影响主流程
                }
            }
        }
        
        /// <summary>
        /// 捕获窗口缩略图
        /// </summary>
        private Image? CaptureWindowThumbnail(IntPtr hWnd)
        {
            // 使用 PrintWindow API 或 BitBlt 捕获窗口
            // 返回缩略图 Image
        }
        
        // 事件处理...
    }
}
```

**UI布局设计（在设计器中创建）**:
```
┌─────────────────────────────────────┐
│ 任务名称: [任务1]          [状态]   │
│ URL: https://example.com            │
│                                     │
│ ┌─────────────────────────────┐   │
│ │                             │   │
│ │   浏览器缩略图 (实时更新)    │   │
│ │                             │   │
│ └─────────────────────────────┘   │
│                                     │
│ [启动] [停止] [编辑] [删除] [显示]  │
└─────────────────────────────────────┘
```

---

#### ScriptTaskFlowLayout（流式布局容器）
```csharp
namespace YongLiSystem.Views.Dashboard.Controls
{
    /// <summary>
    /// 脚本任务流式布局容器
    /// 在设计器中创建，使用 FlowLayoutPanel
    /// </summary>
    public partial class ScriptTaskFlowLayout : XtraUserControl
    {
        private readonly DataCollectionService _collectionService;
        private readonly List<ScriptTask> _tasks = new();
        private readonly Dictionary<int, ScriptTaskCard> _cardMap = new();
        
        public ScriptTaskFlowLayout()
        {
            InitializeComponent(); // 设计器中创建 FlowLayoutPanel
            _collectionService = new DataCollectionService();
            LoadTasks();
        }
        
        /// <summary>
        /// 加载所有任务
        /// </summary>
        private void LoadTasks()
        {
            _tasks.Clear();
            _tasks.AddRange(_collectionService.LoadAllScriptTasks());
            
            // 为每个任务创建卡片
            foreach (var task in _tasks)
            {
                AddTaskCard(task);
            }
        }
        
        /// <summary>
        /// 添加任务卡片
        /// </summary>
        private void AddTaskCard(ScriptTask task)
        {
            var card = new ScriptTaskCard(task);
            card.Width = 300; // 固定宽度
            card.Height = 250; // 固定高度
            card.Margin = new Padding(5);
            
            flowLayoutPanel.Controls.Add(card);
            _cardMap[task.Id] = card;
        }
        
        /// <summary>
        /// 添加新任务
        /// </summary>
        public void AddNewTask()
        {
            // 弹出配置对话框
            using var dialog = new ScriptTaskConfigDialog();
            if (dialog.ShowDialog() == DialogResult.OK)
            {
                var task = dialog.GetTask();
                _collectionService.SaveScriptTask(task); // 自动保存
                _tasks.Add(task);
                AddTaskCard(task);
            }
        }
        
        /// <summary>
        /// 更新任务
        /// </summary>
        public void UpdateTask(ScriptTask task)
        {
            _collectionService.SaveScriptTask(task); // 自动保存
            if (_cardMap.TryGetValue(task.Id, out var card))
            {
                card.UpdateTask(task);
            }
        }
        
        /// <summary>
        /// 删除任务
        /// </summary>
        public void DeleteTask(int taskId)
        {
            _collectionService.DeleteScriptTask(taskId);
            if (_cardMap.TryGetValue(taskId, out var card))
            {
                flowLayoutPanel.Controls.Remove(card);
                _cardMap.Remove(taskId);
            }
            _tasks.RemoveAll(t => t.Id == taskId);
        }
    }
}
```

---

### 4. 浏览器集成

#### ScriptTaskBrowserManager（浏览器管理器）
```csharp
namespace YongLiSystem.Services.Dashboard
{
    /// <summary>
    /// 脚本任务浏览器管理器
    /// 负责管理每个任务的浏览器窗口
    /// </summary>
    public class ScriptTaskBrowserManager
    {
        private readonly Dictionary<int, BrowserWindowProxy> _browserProxies = new();
        
        /// <summary>
        /// 初始化任务的浏览器窗口
        /// </summary>
        public async Task InitializeBrowserAsync(ScriptTask task)
        {
            if (task.BrowserProxy != null)
                return;
            
            var proxy = new BrowserWindowProxy();
            proxy.OnLog += (s, msg) => 
            {
                // 记录日志
            };
            
            await proxy.InitializeAsync($"{task.Name} - 浏览器", task.Url);
            
            task.BrowserProxy = proxy;
            _browserProxies[task.Id] = proxy;
            
            // 如果配置了自动登录，执行登录
            if (task.AutoLogin && !string.IsNullOrEmpty(task.Username))
            {
                await ExecuteLoginAsync(task);
            }
        }
        
        /// <summary>
        /// 执行登录
        /// </summary>
        private async Task ExecuteLoginAsync(ScriptTask task)
        {
            if (task.BrowserProxy == null) return;
            
            // 执行登录脚本
            await task.BrowserProxy.ExecuteCommandAsync("登录", new
            {
                username = task.Username,
                password = task.Password
            });
        }
        
        /// <summary>
        /// 执行采集脚本
        /// </summary>
        public async Task<string?> ExecuteScriptAsync(ScriptTask task)
        {
            if (task.BrowserProxy == null) return null;
            
            var result = await task.BrowserProxy.ExecuteCommandAsync("执行脚本", task.Script);
            return result.Data?.ToString();
        }
        
        /// <summary>
        /// 显示/隐藏浏览器窗口
        /// </summary>
        public void ShowBrowser(ScriptTask task, bool show)
        {
            task.BrowserProxy?.SetVisible(show);
        }
        
        /// <summary>
        /// 释放浏览器资源
        /// </summary>
        public void DisposeBrowser(ScriptTask task)
        {
            if (_browserProxies.TryGetValue(task.Id, out var proxy))
            {
                proxy.Dispose();
                _browserProxies.Remove(task.Id);
            }
            task.BrowserProxy = null;
        }
    }
}
```

---

### 5. 配置对话框

#### ScriptTaskConfigDialog（配置对话框）
```csharp
namespace YongLiSystem.Views.Dashboard
{
    /// <summary>
    /// 脚本任务配置对话框
    /// 使用 MonitorConfigControl 作为配置界面
    /// </summary>
    public partial class ScriptTaskConfigDialog : XtraForm
    {
        private MonitorConfigControl _configControl;
        private ScriptTask? _task; // 如果为 null，表示新建；否则为编辑
        
        public ScriptTaskConfigDialog(ScriptTask? task = null)
        {
            InitializeComponent(); // 设计器中创建
            _task = task;
            InitializeConfigControl();
        }
        
        private void InitializeConfigControl()
        {
            _configControl = new MonitorConfigControl();
            _configControl.Dock = DockStyle.Fill;
            this.Controls.Add(_configControl);
            
            // 如果是编辑模式，加载现有配置
            if (_task != null)
            {
                _configControl.Url = _task.Url;
                _configControl.Username = _task.Username;
                _configControl.Password = _task.Password;
                _configControl.AutoLogin = _task.AutoLogin;
                _configControl.Script = _task.Script;
            }
        }
        
        /// <summary>
        /// 获取配置的任务对象
        /// </summary>
        public ScriptTask GetTask()
        {
            var task = _task ?? new ScriptTask();
            task.Name = $"任务_{DateTime.Now:yyyyMMddHHmmss}";
            task.Url = _configControl.Url;
            task.Username = _configControl.Username;
            task.Password = _configControl.Password;
            task.AutoLogin = _configControl.AutoLogin;
            task.Script = _configControl.Script;
            return task;
        }
    }
}
```

---

## 🔄 工作流程

### 1. 添加任务流程
```
用户点击 buttonAddScriptTask
    ↓
弹出 ScriptTaskConfigDialog（使用 MonitorConfigControl）
    ↓
用户填写配置（URL、用户名、密码、脚本等）
    ↓
点击确定
    ↓
创建 ScriptTask 对象
    ↓
保存到数据库（自动保存）
    ↓
创建 ScriptTaskCard 并添加到流式布局
    ↓
初始化浏览器窗口（BrowserWindowProxy）
    ↓
开始更新缩略图（1秒一次）
```

### 2. 启动任务流程
```
用户点击卡片上的"启动"按钮
    ↓
调用 ScriptTaskBrowserManager.InitializeBrowserAsync()
    ↓
创建 BrowserWindowProxy 并初始化浏览器窗口
    ↓
如果配置了自动登录，执行登录
    ↓
更新任务状态为 Running
    ↓
开始执行脚本（定时或手动）
```

### 3. 缩略图更新流程
```
ScriptTaskCard 启动定时器（1秒间隔）
    ↓
检查 BrowserProxy.WindowHandle 是否有效
    ↓
使用 Windows API 捕获窗口截图
    ↓
生成缩略图（缩放为卡片大小）
    ↓
更新 picThumbnail.Image
    ↓
1秒后重复
```

### 4. 数据保存流程
```
任务对象属性变更
    ↓
触发 ScriptTaskFlowLayout.UpdateTask()
    ↓
调用 DataCollectionService.SaveScriptTask()
    ↓
自动保存到共同的 app.db 数据库（ScriptTasks 表）
```

### 5. 启动时加载流程
```
DataCollectionPage 初始化
    ↓
创建 ScriptTaskFlowLayout
    ↓
ScriptTaskFlowLayout 构造函数调用 LoadTasks()
    ↓
从数据库加载所有任务
    ↓
为每个任务创建卡片
    ↓
如果任务 IsEnabled = true，自动初始化浏览器
```

---

## 📊 数据库表结构

### ScriptTasks 表
```sql
CREATE TABLE ScriptTasks (
    Id INTEGER PRIMARY KEY AUTOINCREMENT,
    Name TEXT NOT NULL,
    Url TEXT NOT NULL,
    Username TEXT,
    Password TEXT,
    AutoLogin INTEGER NOT NULL DEFAULT 0,
    Script TEXT,
    CreateTime TEXT NOT NULL,
    UpdateTime TEXT NOT NULL,
    IsEnabled INTEGER NOT NULL DEFAULT 1
);
```

---

## 🎨 UI 设计要点

### 1. ScriptTaskCard 布局（在设计器中创建）
- **尺寸**: 300x250 像素
- **布局**: 使用 LayoutControl 或 TableLayoutPanel
- **控件**:
  - Label: 任务名称
  - Label: URL（可截断显示）
  - Label: 状态
  - PictureBox: 浏览器缩略图（200x150）
  - SimpleButton: 启动、停止、编辑、删除、显示浏览器

### 2. ScriptTaskFlowLayout 布局（在设计器中创建）
- **容器**: FlowLayoutPanel
- **方向**: FlowDirection.LeftToRight
- **换行**: WrapContents = true
- **间距**: 5 像素

### 3. groupCustTask 布局
- **容器**: GroupControl
- **内容**: ScriptTaskFlowLayout（Dock = Fill）
- **标题**: "[半自动]定制采集任务"

---

## 🔧 技术实现细节

### 1. 窗口截图（缩略图生成）
```csharp
[DllImport("user32.dll")]
private static extern bool PrintWindow(IntPtr hWnd, IntPtr hdcBlt, int nFlags);

private Image? CaptureWindowThumbnail(IntPtr hWnd)
{
    if (hWnd == IntPtr.Zero) return null;
    
    var rect = new RECT();
    GetWindowRect(hWnd, out rect);
    
    int width = rect.Right - rect.Left;
    int height = rect.Bottom - rect.Top;
    
    if (width <= 0 || height <= 0) return null;
    
    using var bmp = new Bitmap(width, height);
    using var g = Graphics.FromImage(bmp);
    var hdc = g.GetHdc();
    
    PrintWindow(hWnd, hdc, 0);
    g.ReleaseHdc(hdc);
    
    // 缩放为缩略图大小
    var thumbnail = new Bitmap(200, 150);
    using (var g2 = Graphics.FromImage(thumbnail))
    {
        g2.DrawImage(bmp, 0, 0, 200, 150);
    }
    
    return thumbnail;
}
```

### 2. SQLite 操作（使用共同的数据库）
- 使用 `SQLite` 命名空间（`using SQLite;`）
- 使用 `SQLiteConnection` 连接共同的 `app.db` 数据库
- 使用 `CreateTable<T>()` 创建 `ScriptTasks` 表（新增表，与现有表共存）
- 使用 `Insert()` / `Update()` / `Delete()` 操作数据
- 数据库路径：`C:\Users\[用户名]\AppData\Local\永利系统\Data\app.db`

### 3. 浏览器窗口管理
- 使用 `BrowserWindowProxy` 管理浏览器窗口
- 每个任务独立一个浏览器窗口
- 浏览器窗口运行在独立 STA 线程
- 通过 `ExecuteCommandAsync()` 执行脚本

---

## ✅ 实现检查清单

### 数据模型
- [ ] 创建 `ScriptTask` 类
- [ ] 创建 `ScriptTaskStatus` 枚举
- [ ] 添加 `[Ignore]` 属性标记运行时属性

### 数据库层（集成到 DataCollectionService）
- [ ] 在 `DataCollectionService` 中添加 `InitializeScriptTasksTable()`
- [ ] 在 `DataCollectionService` 中添加 `LoadAllScriptTasks()`
- [ ] 在 `DataCollectionService` 中添加 `SaveScriptTask()`
- [ ] 在 `DataCollectionService` 中添加 `DeleteScriptTask()`
- [ ] 确保使用共同的 `app.db` 数据库文件

### UI 控件（设计器中创建）
- [ ] 创建 `ScriptTaskCard` 用户控件
- [ ] 创建 `ScriptTaskFlowLayout` 用户控件
- [ ] 创建 `ScriptTaskConfigDialog` 对话框

### 浏览器集成
- [ ] 创建 `ScriptTaskBrowserManager`
- [ ] 实现浏览器初始化
- [ ] 实现脚本执行
- [ ] 实现缩略图捕获

### 集成到页面
- [ ] 在 `DataCollectionPage` 中添加 `ScriptTaskFlowLayout`
- [ ] 绑定 `buttonAddScriptTask` 点击事件
- [ ] 实现启动时自动加载

---

## 📝 注意事项

1. **设计器优先**: 所有 UI 控件必须在设计器中创建，不要用代码创建
2. **自动保存**: 任务添加、修改后立即保存到数据库
3. **资源管理**: 浏览器窗口需要正确释放，避免内存泄漏
4. **线程安全**: 缩略图更新需要在 UI 线程执行
5. **性能优化**: 缩略图更新频率为 1 秒，避免过于频繁

---

## 🎯 后续扩展

1. **任务调度**: 支持定时执行脚本
2. **脚本模板**: 提供常用脚本模板
3. **任务分组**: 支持任务分组管理
4. **执行历史**: 记录脚本执行历史
5. **通知提醒**: 脚本执行结果通知

---

**设计完成，等待确认后开始实现** ✅
