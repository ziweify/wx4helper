    ...
    public partial class UcBingoDataCur : UserControl
    {
        ...
        private void UpdateDisplay()
        {
            ...
            UpdateUIThreadSafe(() =>
            {
                ...
                if (issueId > 0)
                {
                    var openTime = BingoHelper.GetIssueOpenTime(issueId);
        ...
        {
            Console.WriteLine($"📢 UcBingoDataCur 收到期号变更事件: {e.OldIssueId} → {e.NewIssueId}");
            ...
            if (_lotteryService != null)
            {
                UpdateUIThreadSafe(() =>
                {
                    ...
                    // 计算并显示开奖时间
                    var openTime = BingoHelper.GetIssueOpenTime(e.NewIssueId);
                    lblOpenTime.Text = openTime.ToString("HH:mm:ss");
                    
                    Console.WriteLine($"✅ UcBingoDataCur 已更新: 期号={e.NewIssueId}, 时间={openTime:HH:mm:ss}");
                });
            }
        }
        
        private void OnStatusChanged(object? sender, BingoLotteryStatusChangedEventArgs e)
        {
            UpdateDisplay();
        }
        
        private void OnCountdownTick(object? sender, BingoLotteryCountdownEventArgs e)
        {
            UpdateDisplay();
        }
